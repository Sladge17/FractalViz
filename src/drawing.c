...
void	draw_axis(t_sys *sys)
{
	...
	while (i + sys->scale[1][(int)sys->index] * j < i + (WIDTH - MENU_W) / 2)
	{
		n = 0;
		while(n < 7)
		{
			sys->imgout[i + n * (WIDTH - MENU_W) + sys->scale[1][(int)sys->index] * j] = AXIS_C;
			sys->imgout[i + n * (WIDTH - MENU_W) - sys->scale[1][(int)sys->index] * j] = AXIS_C;
			n += 1;
		}
		j += 1;
	...
	while (i + j * (WIDTH - MENU_W) * sys->scale[1][(int)sys->index] < sys->imgvol)
	{
		n = 0;
		while(n < 7)
		{
			sys->imgout[i + (WIDTH - MENU_W) * sys->scale[1][(int)sys->index] * j + n] = AXIS_C;
			sys->imgout[i - (WIDTH - MENU_W) * sys->scale[1][(int)sys->index] * j + n] = AXIS_C;
			n += 1;
		}
		j += 1;
